...
class CertificateController extends Controller
{
  ...
  public function barangayClearance($id)
  {
    ...
    if ($certificate->status == 'pending') {
      ...
      ActivityLog::log(
        ' approved the ' . $certificate->doctype .' requested by '. $certificate->fullname,
        'certificate',
        $certificate->id,
      );
    ...
    return view('admin.requestsCertificates.barangayClearance', ['cer' => $cer,'certificate' => $certificate,'barangay_secretary'=>$barangay_secretary,'barangay_head'=>$barangay_head]);
  }
  ...
  public function barangayResidency($id)
  {
    ...
    $barangay_head = barangayOfficial::where('position','Barangay Captain')->first();
    $barangay_secretary = barangayOfficial::where('position','Barangay Secretary')->first();
    return view('admin.requestsCertificates.barangayResidency', ['cer' => $cer,'certificate' => $certificate,'barangay_secretary'=>$barangay_secretary,'barangay_head'=>$barangay_head]);
  }
  ...
  public function barangayIndigency($id)
  {
    ...
    $barangay_head = barangayOfficial::where('position','Barangay Captain')->first();
    $barangay_secretary = barangayOfficial::where('position','Barangay Secretary')->first();
    return view('admin.requestsCertificates.barangayIndigency', ['cer' => $cer,'certificate' => $certificate,'barangay_head'=>$barangay_head,'barangay_secretary'=>$barangay_secretary]);
  }
  public function deleteRequest($id)
  {
    ...
    ActivityLog::log(
      ' deleted the ' . $cer->doctype .' which was requested by ' . $cer->fullname,
      'certificate',
      $cer->id,
    );
  ...
  public function declineCertificate($id)
  {
    ...
    if ($certificate->status == 'pending') {
      ...
      ActivityLog::log(
        ' declined the' . $certificate->doctype . ' requested by ' . $certificate->fullname,
        'certificate',
        $certificate->id,
      );
    }
  ...
  public function changeCertStatus(Request $request){
    $url = route('changeStatus');
    $certs = RequestCertificate::find($request->clearance_id);
    $certs->status2 = $request->status2;
    $certs->save();
