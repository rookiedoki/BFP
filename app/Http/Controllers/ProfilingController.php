...
public function vaccProfiling(){
  $vacc = Vaccine::where('status', '=', '0')->paginate(10);
...
public function seniorProfiling(){
  $sen = SeniorCitizen::where('status', '=', '0')->paginate(10);
...
//Add to List Pregnant
public function addListPregnant(Request $request, $id){
  $pregProfiling = Pregnant::find($id);
  $status = 1;
  Pregnant::where('id', $id)->update(['status' => $status]);
  return redirect('/listPregnant')->with('message', 'Add to List Completed');
}
...
public function viewPWDProfiling($id){
  ...
  return view('Admin.Profiling.PWD.viewPWDProfiling', ['pwdProfiling'=>$pwdProfiling]);
    ...
    public function deleteVacc($id)
    {
        ...
        $Vacc->delete();
        return back()->with('message', 'Resident Record Succesfully Deleted');
    }

       //Delete Vaccinaton
       public function deletePreg($id)
       {
           $preg=Pregnant::find($id);
           $preg->delete();
           return back()->with('message', 'Resident Record Succesfully Deleted');
       }
        ...
        public function deleteSen($id)
        {
            $sen=SeniorCitizen::find($id);
            $sen->delete();
            return back()->with('message', 'Resident Record Succesfully Deleted');
        }
...
public function listPregnant(){
  $preg = Pregnant::all()->where('status', '=', '1');
  return view('Admin.Profiling.Pregnant.listPregnant',['preg'=>$preg]);
}

//List of Senior Citizen Profiling
public function pwdProfiling(){
  $pwd = PWD::where('status', '=', '0')->paginate(10);
  return view('Admin.Profiling.PWD.pwdProfiling',['pwd'=>$pwd]);
}

//List of Pregnant Profiling
public function pregnantProfiling(){
  $preg = Pregnant::where('status', '=', '0')->paginate(10);
  return view('Admin.Profiling.Pregnant.pregnantProfiling',['preg'=>$preg]);
}

//View Residence Vaccination Profiling
public function viewPregnantProfiling($id){
  $pregProfiling = Pregnant::find($id);
  return view('Admin.Profiling.Pregnant.viewPregProfiling', ['pregProfiling'=>$pregProfiling]);
}
